...
The scientific name should be the name in lowest level taxonomic rank that can be determined, which is the species name (genus + specificEpithet) here. This information is a compilation of `genus` and `species` in `raw_data`. We first generate `scientificName` by combining `genus` and `species`.
