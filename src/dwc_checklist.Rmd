...
Within the `INFORMAL` group, we clean the data by removing the specificEpiphet `agg.` and `sp.` (which is the same thing as using the column `genus` for `scienticName`):
...
Within the `HYBRID` group, some species also need some cleaning:

```{r}
raw_data %<>% mutate(scientificName_clean = recode(scientificName_clean,
  "Brachyglottis x 'Sunshine'" =  "Brachyglottis 'Sunshine'",
  "Cotoneaster x 'Hybridus pendulus'" = "Cotoneaster 'Hybridus pendulus'",
  "Fuchsia x 'Corallina'" = "Fuchsia 'Corallina'",
  "Symphytum x 'Hidcote Blue'" = "Symphytum 'Hidcote Blue'"))
```

Result of the cleaning step:

```{r}
raw_data %>% select(scientificName, scientificName_clean, type) %>% 
  filter(type == "INFORMAL" | type == "HYBRID") %>% 
  arrange(type)
```
...
# Generate taxonID:
raw_data %<>% mutate(taxon_id = paste("rinse-checklist", "taxon", vdigest(scientificName, algo="md5"), sep = ":"))
```
